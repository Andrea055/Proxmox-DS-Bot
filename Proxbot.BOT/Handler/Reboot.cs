    ...
    public class Reboot{
        async public void Handler(SocketMessageComponent component, string info ){
            ...
            if(user.GuildPermissions.Administrator){
                ...
                var id = raw_data[0].Replace("rebootID:", "");
                ...
                {
                    var stop_raw = await client.Nodes[node].Qemu[id].Status.Reboot.VmReboot();
                    await component.RespondAsync("VM rebooted successfully");
                }
            }else{
