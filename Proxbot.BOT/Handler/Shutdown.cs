    ...
    public class Shutdown{
        async public void Handler(SocketMessageComponent component, string info ){
            ...
            if(user.GuildPermissions.Administrator){
                ...
                var id = raw_data[0].Replace("shutdownID:", "");
                ...
                {
                    var stop_raw = await client.Nodes[node].Qemu[id].Status.Shutdown.VmShutdown();
                    await component.RespondAsync("VM shuts down successfully");
                }
            }else{
